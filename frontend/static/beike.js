...
<div class="bk-foot">
  ...
  <details class="bk-method"><summary>数据怎么来的?</summary>
    <ul>
      ...
      <li><b>听力第三方核验</b> — 2021–26 音频已入 <code>data/audio</code> 且 ASR 指纹过门; 源非 NEEA/省考试院官方发行, UI 可播放须标明第三方。</li>
...
<span class="bk-suff ${suffClass}">${suffLabel}</span>${banner}`;
  ...
  function renderTrend(p) {
    ...
    G.$("#bk-trendnote").innerHTML = `<b>结构真值</b>(题型存续时间带 · 登退场信号由<b>卷面结构</b>定非数据): `
      ...
      + `<br><small class="muted">注 淡色虚线带·段=提取不全或该年未登记(${gaps.join("、") || "无"}): 卷面常驻/确有但本项目未抽全 → <b>存续年仅样本, 不作首末考年信号</b>(听力=骨架题型·题干提取仍有缺口, ≠登场2021; 续写真登场但登场年不可信); 空心点=登/退场年不可信, 只作卷面级信号。听力音频=第三方核验可播(非 NEEA)。</small>`;
  }
