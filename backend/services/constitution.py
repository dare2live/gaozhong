...
IRON_LAWS = [
    ...
    ("P16", "iron_law", "解析必须对应原文",
     "阅读: 标注原文段落号+具体句子; 推理题: 额外给出逻辑链; 听力: 标注transcript对应句; 全局讲解引用教材出处+真题题号.",
     "ground_truth_validator 检查解析是否含原文引用", "§3.1"),
    ("P17", "iron_law", "重写优于保留",
     "模型训练完成后, 不符合模型输出标准的旧内容必须重写, 不保留凑数内容. 以模型输出为唯一标准.",
     "batch_regression_test 检测重写前后对齐度", "§5"),
]
