...
TERM_TO_LABEL_KEYWORD = {
    ...
    "人称代词":   "人称代词",
    "一般过去":   "一般过去",
    # 2026-07-07 坑31修复: "非谓语"原值"非谓语"在grammar_items无精确label(实际label=
    # "动词的非谓语形式"), 旧版kw-in-label子串规则能凑巧命中, 改精确匹配后须订正为真实label。
    "非谓语":     "动词的非谓语形式",
}

# 2026-07-07 坑31 (子串跨枝/跨层误配, workflow独立复算实证发现): 旧版 `kw in label` 全局子串对
...
def match_ids_for_term(items: list[tuple], term: str, kw: str) -> set[str]:
    """term → grammar_item_id 精确匹配 (替代旧版全局 kw-in-label 子串, 坑31)。
