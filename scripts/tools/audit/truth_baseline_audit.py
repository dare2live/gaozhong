...
def _init_year_summary() -> dict[int, dict[str, Any]]:
    return {y: {"db_count": 0, "truth_count": 0, "matched": 0, "db_only": 0, "truth_only": 0} for y in TARGET_YEARS}


def _accumulate_recon_counts(summary: dict[int, dict[str, Any]], recon: dict[str, Any]) -> None:
...
def _count_db_have_question_bank(recon: dict[str, Any], bank_ids: set[str], y: int) -> int:
    count = 0
...
def _count_db_question_bank_total(recon: dict[str, Any], y: int) -> int:
    count = 0
    for row in recon["matched"]:
        if row["year"] == y:
            count += 1
...
def make_year_summary(recon: dict[str, Any], bank_ids: set[str]) -> dict[int, dict[str, Any]]:
    summary = _init_year_summary()
    _accumulate_recon_counts(summary, recon)
    for y, s in summary.items():
        ...
        s["db_have_question_bank"] = _count_db_have_question_bank(recon, bank_ids, y)
        s["db_question_bank_total"] = _count_db_question_bank_total(recon, y)
...
def _load_initial_records(con: "duckdb.DuckDBPyConnection") -> tuple[list[dict[str, Any]], set[str], list[dict[str, Any]]]:
    ...
    return db_records, bank_ids, truth_records


def _compute_recon_bundle(
    db_records: list[dict[str, Any]], truth_records: list[dict[str, Any]], bank_ids: set[str]
    ...
    return recon, summary, rows


def _build_report_dict(
    *,
    summary_after: dict[int, dict[str, Any]],
    rows_after: list[dict[str, Any]],
    db_records_after: list[dict[str, Any]],
    truth_records: list[dict[str, Any]],
...
def _write_cross_verify_reports(
    report: dict[str, Any], rows_after: list[dict[str, Any]], summary_after: dict[int, dict[str, Any]],
    run_id: str, manifest: Any, args: argparse.Namespace,
) -> None:
...
def _print_summary(report: dict[str, Any], out: Path, md_out: Path, args: argparse.Namespace) -> None:
    status = report["status"]
    findings = report["findings"]
    data_gap = report["gaps_to_target"]
    ...
    if args.import_missing:
        print(f"  inserted={report['import']['inserted_rows']}")
    if data_gap:
...
def main() -> None:
    ...
    try:
        db_records, bank_ids, truth_records = _load_initial_records(con)
        if args.import_missing:
            con.close()
    ...
    recon, _summary, _rows = _compute_recon_bundle(db_records, truth_records, bank_ids)
    ...
    recon_after, summary_after, rows_after = _compute_recon_bundle(db_records_after, truth_records, bank_ids_after)

    run_id, manifest = manifest_hash()
    report = _build_report_dict(
        summary_after=summary_after,
        rows_after=rows_after,
        db_records_after=db_records_after,
    ...
    _write_cross_verify_reports(report, rows_after, summary_after, run_id, manifest, args)
    _print_summary(report, out, md_out, args)
