...
- ✅ **M0 已完成项**：识别到 `2024/2025` PDF 可核对源，基础导入已验证成功（`import_recent_exams.py` 已跑通，cross_verify 通道可用）。
- 🔶 **M0 未完成项**：`2021/2022` 新高考 II 卷仍需完整入库与污染剔除（目标每年 55–60 题，含解析及题型）。
- 🔲 **M1~M3**：按顺序等待 M0 关闭后启动。
