...
| M | 名称 | 验收 | 状态 |
|---|---|---|---|
| **M0** | 资料基石 + 框架 + 顶层架构 | DB 多表, API 11 endpoint, 前端 10 区块, 6 类审计 0 FAIL | ✅ |
| **M1** | 一城一册全程数据通 (沈阳/外研必修一) | 单元 + section + 词表 + 真题映射 链路完整, 任意 unit 能查到 "学过哪些词/语法 + 历年真题考过哪些" | 🟡 70% (vocab/units OK, section 待) |
| **M2** | L1 课时题生成 (词汇 + 短句) | 任 1 unit 出 5 道单选 + 答案 + 考点 graph | ✅ PoC (词汇选义题) |
| **M3** | L2 单元题 + L3 阶段卷 | 单元卷 30 题, 阶段卷 80 题, 比例对齐辽宁卷 | ⬜ |
| **M4** | L4 模拟卷 (完整 150 分 / 120 min) | 题型分布 = 真卷, 越纲率 < 5% | ⬜ |
| **M5** | 学生交互界面 + 弱点反馈 | 登入 / 答题 / 错题集 / 个性化推荐 | ⬜ |
