...
- **[地基最后一轴, 已实证]** ① 语法 per-unit: grammar_items 106(课标官方✓) 但 **grammar_occurrences=0**(从未建)。19 单元语法段主题全英文, 无法 string-match 中文课标(naive 匹配=卷尾附录假阳性垃圾, D0 返空>假推不落库)。**修法**: curated 英文主题→课标项 YAML(Modals→情态动词 等标准等价非估算) + Grammar section 标题提取主题 + provenance, 无清晰匹配留 NULL。需谨慎设计非仓促(详 data_accuracy_audit 2026-06-16 语法缺口段)。
- **[可自动推进]** ② 备课整合(unit→词/语法/考点/关联 一体视图, 把三大件+地基接成教师备课闭环) ③ 件3前端浏览器实测(preview截图) ④ 任务#15 解析采集(低优先)。
