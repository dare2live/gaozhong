...
把 2026-06-10 起的开发按「数据真相优先、可复算、单次交付」推进。  
本阶段不做微步提交，不分散到零碎修复；每个里程碑必须一次性输出完备证据并一次完成状态流转。

### 里程碑执行纪律（本文件约束）

1. **先证据、后动作、后验收**：每个里程碑先冻结输入，再执行所有改造，最后一次性跑 `scripts/data_accuracy_check.py` 与 `bash scripts/stop_gate.sh`。
2. **里程碑状态单向流转**：`未开始 -> 进行中 -> 完成`，不允许回退/反复切换。
3. **一个里程碑只跑一次“主脚本链”**：不允许同一目的的反复小修复式重跑。
4. **可复算**：每个关键报告都带 `run_id`，输入参数、命令、输出哈希（去掉时间戳）入档，可复查。
5. **审计先行阻断**：任何 `FAIL` 或 `WARN` 触发即回退到里程碑头尾并修复，不进入下一阶段。
...
| M3 审计与交付闭环 | `🔶 进行中（待反馈闭环）` | `run_id=20260610T074134Z` |

### 本轮开发包（不分小步）

## Milestone C（核心）—内容与题库质量闭环（M2）

### 目标

一次性完成 `question_bank` 与 `course` 的“可追溯教学闭环”，同时修正已确认的数据口径问题：
- `rule_synth` 无 `analysis` 的题目整体清退与可追溯替换（不做局部打补丁）。
- `course_materials.ref_id` 与 `nodes` 关联规则统一为 `question:<id>` 映射。
- 题库/课程链路 audit 全部支持版本化报告，不留下孤儿题、断链教材位置信息。

### 一次性交付包（建议按这三包一次提交）

**包 1：题库真实口径修复**
- 输入：`question_bank` 全量（含 `rule_synth`）+ 已有课程引用关系表。
- 处理：一次性完成 `rule_synth` 规则清单、候选替换策略、入库变更落盘。
- 输出：
  - `rule_synth` 退役列表 + 替换原因（按题型/长度/来源聚合）
  - `analysis` 缺失率从基线到 0 的变更快照
  - 新旧 question_id 映射表（用于问题可追溯）

**包 2：课程材料引用一致化**
- 输入：`backend/services/question_bank` 与 `backend/services/course/materials.py` 相关映射链路，`course_materials` 与 `nodes` 对齐规则。
- 处理：一次性将所有 `source='auto_from_trend'` 且未携带 `question:` 前缀的 `ref_id` 标准化，补齐 `question:` 映射。
- 输出：
  - `course_materials` 映射一致性报告（通过率、修复率、失败样本）
- 验收：
  - 课程一致性 audit 中 `exam_question` 相关检索不再出现前缀错配导致的“假孤儿”

**包 3：跨链路闭环验证与冻结**
- 输入：已修复的 `question_bank` + `course` + 现有 `manifest`。
- 处理：一次性拉齐 `R2/R3/R4/R5/R6 + data_accuracy_check` 对应检查链路与教师端入口可用性。
- 输出：
  - M2 里程碑验收报告（含时间戳、run_id、核心指标、失败清单）
  - `docs/data_accuracy_audit.md` 的对应行更新
- 验收：
  - 课程与题库关键约束项全部转为 OK 或留明确替代说明
  - 无新 WARN/FULL FAIL 阻塞项

### M2 关键验收条件（单次判定）

1. `question_bank` 中 `analysis` 全量率提升且 `rule_synth` 相关异常可追溯归档（含入库/替换决策）。
2. `course_materials.ref_id` 与 `nodes` 的问答引用规则统一；课程一致性检测不再报“纯数字 ref_id 造成的伪孤儿”。
3. `scripts/data_accuracy_check.py` 与 `bash scripts/stop_gate.sh` 同时通过。
4. `docs/data_accuracy_audit.md` 与 `goal.md` M2 区块完全同步（含 run_id/报告路径）。
...
- 关键结论：
  ...
  - `/app` 人验项清单已生成并补齐 V1~V8 feedback；V8 打印能力以源码证据闭环。
...
1. 每个里程碑只允许一条主链路：`冻结输入 -> 全量执行 -> 全量验收 -> 文档冻结 -> 状态更新`。  
2. 任何 `FAIL` 或新增 `WARN` 直接阻断并回到里程碑起点，不允许跨里程碑跳步。  
3. 每个里程碑只保留一次冻结产物：`json|md 报告 + run_id + 可复算说明 + docs 状态同步`。  
4. `M3` 以真人反馈/替代闭环为硬条件；`M4/M5` 只能在 `M3` 达成后启动。  

### 当前里程碑状态（以本文件为单一真相源）

| 里程碑 | 状态 | 最新 run_id | 触发条件 |
|---|---|---|---|
| M0 真值基座闭环 | ✅ 已完成 | `b3fd3dc87989be20` | 真值链路可复核 |
| M1 图谱与趋势闭环 | ✅ 已完成 | `d0b83b4ef781d247` | 真值驱动趋势与图谱可复现 |
| M2 内容与题库质量闭环 | ✅ 已完成 | `20260610T073936Z` | 口径统一 + analysis 追踪闭环 |
| M3 审计与交付闭环 | 🔶 进行中（待反馈闭环） | `20260610T074134Z` | 仅在反馈录入并可复算后可转完成 |
| M4 课程主链路交付闭环 | ⬜ 未开始 | - | 需先完成 M3 |
| M5 运营试运行闭环 | ⬜ 未开始 | - | 需先完成 M4 |

### 本会话开发路线（严格非小步）

#### Milestone 1：M3 一次性收口（目标窗口 2026-06-10 ~ 2026-06-16）

**目标**：把 `M3` 从“状态进行中”一次性变为“已完成”，同时不引入新功能。  

**一次性交付包**  
- 用现有 `run` 与脚本主链路完成 `data_accuracy_check + stop_gate + verification_protocol` 的统一回放。  
- 统一补齐 `docs/user_test_round1.md` 与 `docs/teacher_feedback_round1.md`：  
  - 有真实反馈则写实录入；无反馈则按约定替代闭环+可执行补录计划写入。  
- 在 `data/reports/m3_closure_*.json|md`、`data/reports/verification_protocol.json`、`docs/data_accuracy_audit.md`、`goal.md` 中同步同一状态（run_id + 证据路径 + 结论）。  

**验收门槛（一次性）**  
- `scripts/data_accuracy_check.py` 与 `bash scripts/stop_gate.sh` 必须均为 PASS。  
- `data/reports/verification_protocol.json` V1~V8 必须为 DONE 或明确记录 `待补录边界 + 解决时点`，不得长期 pending。  
- `goal.md` 与 `docs/data_accuracy_audit.md` 一致且可复算（含命令、输入、输出摘要）。  

**本阶段输出目标**：`M3` 状态更新为 `已完成`，产出 `run_id=20260610T074134Z` 的冻结说明。

#### Milestone 2：M4 一次性交付（目标窗口 2026-06-17 ~ 2026-07-07）

**目标**：一次性完成课程主链路教学闭环，确保“可教、可查、可复用”。  

**一次性交付包**  
- 固化 `course_templates.yaml` 课程内容与 `course_materials` 映射口径，`courses=40`、`course_materials` 结构稳定。  
- `R1~R6` 全量重跑并补齐差距（不回退到小补丁），形成 M4 复盘报告。  
- `/app` 端课程主流程、作业流程、弱点推送流程形成一体化演示清单（单页可重放）。  

**验收门槛（一次性）**  
- `R1~R6` 全部可解释、无阻断 FAIL；可解释 WARN 有明确时限。  
- 课程资料、题库、图谱、前端入口形成可追溯 trace：课程→知识点→题目→结果。  
- 生成 `M4` 冻结报告（json+md+run_id）并同步回 `docs/data_accuracy_audit.md` 与 `goal.md`。

#### Milestone 3：M5 运营试运行（目标窗口 2026-07-08 ~ 2026-07-14）

**目标**：形成可持续交付、可复用的运营起始态。  

**一次性交付包**  
- 形成完整启动与巡检手册：启动、备份、恢复、告警、周报模板。  
- 完成 1 次连贯的老师/学生关键流程演练并补档（可替代为 2 次内部演练，但必须可复算）。  
- 冻结 `weekly health check` 与 `stop_gate` 周检机制，确认无新增阻断。  

**验收门槛（一次性）**  
- 演练链路一次通到底，不出现阻断。  
- 一周复测 `FAIL=0` 且 WARN 有明确收口节奏。  
- 运营交付包入 `data/reports/`，在 `goal.md` 与 `docs/data_accuracy_audit.md` 形成一致状态。

### 本会话硬规则（每次状态更新时必须满足）

1. 里程碑不能前移状态：`进行中 -> 已完成` 只能在本里程碑验收全部通过时触发。  
2. 一次里程碑只读一次“主脚本链”；不接受在未完成前的片段式交付。  
3. 所有报告与快照必须带 `run_id`，并保留路径 + 生成参数，避免“文档先行、证据缺失”。  
