...
**命题趋势 / 方向性指引 / 造题 — 红线修订 (2026-06-16 用户立法; 原表述把"押具体题目"过度外延到"任何命题趋势分析", 已纠正)**:
- ✅ **核心竞争力 (做, 且优于人工直觉)**: 数据驱动的**命题趋势分析 + 方向性重点指引** — 哪些考点/题型/题材/命题方式在升温/转变 → 给教学优先级与复习重点。这是好老师本就在做的事(画重点/析趋势), 大数据/ML 做得更准更全, 是项目存在理由之一(见记忆 feedback-core-competency)。
- 🔴 **死亡条款 (banned, 不变)**: 声称押中**具体题目/篇章/答案内容** ("今年考某篇" / "AI押中高考精准预测") = fraud, 谁都不可能, 营销话术。
- 🛡 **诚实护栏 (这是原红线真正的内核, 全部保留, 非禁止分析而是别撒谎)**: ① **样本量诚实** — 趋势只在样本支撑处下结论; 辽宁逐年<10标"不足"不冒充 slope(件1); ② **provenance** — 趋势溯真相源不臆造; ③ **结构≠内容** — 方向性指引=考点/题材/命题方式优先级, 非具体内容预测; 造题=结构对齐+课标合规, 不复现真题; ④ **ML 用对地方** — 数据足才上模型, 不在薄数据硬跑(ChunkyMonkey 教训), 数据厚了当用则用。
- 造题选题可借鉴雅思真源(TIME/科学美国人/TED)话题方向但文本必降级到课标3500词。
...
### 2026-06-16 (续) 备课整合 + 颗粒度对齐第三级 + vision/红线立法 (主线收口)
- **备课整合 ✅** (commit 6c90aa2): unit→词/语法/考点/对齐 一体视图; 单一计算点收口(vocab) + 越层债清(graph.neighbors) + 全序确定性(40k并发零分歧) + 语法轴province锚定. preview 实测 live 可用(词按高考频次/语法定语从句4真题/趋势诚实banner/4路追溯). D0维度24 + moth.
- **plan_cycle hook ✅** (commit 959a52b): 固化"改前计划→工具审计→开工→做完三门审计"(UserPromptSubmit 注入). 补"改前计划"流程缺口.
- **考点颗粒度对齐课标第三级 ✅** (commit 502d01c): theme_l3 35子主题, dual-model(opus+sonnet)标注88一致/67诚实跳过; 20节点/88边/20桥, D0维度22扩. 揭示: 环境污染治理 旧课标0→新高考17.2%暴增. preview 实测 live.
- **命题方式探索 (parked, 用户裁 C: 不是主线)**: 30辽宁解析grounded 设问类型(推断63%隐蔽类); 跨era设问结构稳定(文档实证). **机制轴受解析数据约束**(外省0解析/付费题库墙), 锚现有30解析不强采. 可点命题方式轴需设问级节点(grain fork)+解析, 暂缓. 详 docs/kg_interactive_vision_and_forks.md.
- **跨卷型内容对标 (探索, 验证课标共因)**: 外省284题dual-model内容标注(genre一致127/theme110, waisheng_content_labels.jsonl, 不入辽宁图避污染§7). L1分布全卷型相似度0.91-0.99 → **课标共因主导**(人与社会~50%到处主导), 非卷间借鉴; 辽宁人与自我36%高于全国卷(立德树人个人维度). 方法=描述性对标非ML预测.
- **红线立法修订 ✅** (用户): "命题趋势分析+方向性重点指引=核心竞争力(优于人工)", 删"把趋势分析当押题"的过度外延; 保留4护栏(样本量诚实/provenance/结构≠内容/ML数据足才上). 死亡条款=只"押具体题目内容". 详本文上方"红线修订"段 + gaozhong-ops skill 坑14.

### 下一步计划 (2026-06-16 更新)
- **[最高杠杆, 待用户]** 消费者验证(任务#14): 数据底座(真题分析三大件 + 教材地基100%)已就位, 真老师30分钟试用 = 比继续自动建设更高杠杆的独立校验。Rule10 people-not-agents 需用户动员。
- **[地基最后一轴 ✅ 已建 2026-06-16, commit 26d22db]** 语法 per-unit: grammar_topic_map.yaml(教材主题→课标项 curated 映射) + grammar_occurrence.py; 26 Grammar段→18命中/8诚实跳过, 接 init_db 可复现 + D0 + moth29。**教材地基四轴(units/词表/section/语法)全完整**, §1.2 语法≤已学单元现可机器执行。
- **[可自动推进]** ① 备课整合(unit→词/语法/考点/关联 一体视图, 把三大件+地基接成教师备课闭环, 最高价值: 地基四轴+真题三大件已全就位) ② 件3前端浏览器实测(preview截图) ③ 任务#15 解析采集(低优先)。
...
**用户 2026-05-24 决策** (部分已演进):
- ⏸️ ~~跳过 6.F Docker 部署~~ → 现为交付门硬项, AI 可补 Dockerfile(待形态拍板)
...
## 初中 + 中考子系统 → K12 分阶段平台 (立项 2026-05-25, 重定位+落地 2026-06-17)

> ⚠️ **本段 2026-06-17 重写**: 定位从"储备数据"升级为 **"初中+高中统一 K12 分阶段教学平台"**;
> 教材主用版从"人教/外研假设"**纠正为沪教牛津**(研究证实沈阳实际用版)。
> 现行进度/真相源以 **docs/RESUME.md** + **docs/junior_high_subsystem_design.md** 为准, 本段只留定位锚点。

**定位 (用户 2026-06-17 拍板) = 服务沈阳本市**:
- 教材主用版 = **沪教牛津(广深沈通用, 上海教育出版社)** — 沈阳初中实际用版(§1.4 4源: 辽宁批文[2018]3+Oxford+六三制+美英桥)。
  ⚠️ 辽宁14市初中三分: 沈阳=沪教牛津 / 大连·营口=外研 / 余11市=人教。**绝不复用高中二分推初中**(gaokao卷型坑)。
- 中考 = **沈阳卷, 2024起辽宁省统一命题**(对接高考方向, PIT断点); ≤2023自主命题仅分布快照不做趋势(坑12)。

**核心原语 = stage 维度** (用户洞察: with/the 是义务教育词非高中词, 标 stage 后处理方便):
小学(课标二级) < 初中(课标三级∪沪教) < 高中必修 < 高中选修; **双向贯通**(向下兼容+向上受控渗透+跨阶段语义扩展如 power→power plant)。

**已落地 (Phase 0-2.5, 详 RESUME §B-E)**:
- 沪教牛津6册下载+核验 (data/junior_high/textbooks/hujiao/); 义务课标2022 抽取(词1647+语法66);
  沪教词表926; S4 reconcile 细分高中4329词 stage(1763更精细) + 298语义扩展候选; OCR全局持久(~/.venvs/ocr)。
- 真相源存放: `data/junior_high/{textbooks/hujiao, curriculum, structured/*.jsonl, manifest}`。

**A/B/C 推进中** (architect 重新梳理后, 详 RESUME 下一步): A.Phase3集成+junior独立DB / B.语义扩展边挖掘 / C.沈阳中考真题。
旧用途(高考超纲基准校准)并入 stage 体系 — 初中已知词 = 小学+初中 stage, 高中真超纲 = 校本超纲 stage。
