...
Week 4:  7.12 细粒度重建 (2024/2025 数据回来后) ← 当前
         ...
         │   ├─ 年份权重按宪法 P1 (2025=5)
         │   └─ 回测目标: Spearman ≥ 0.90
         ├─ Phase D: 全量内容重写/优化 (模型训练完成后统一触发)
         │   ├─ D1. 答案解析重写 — 必须对应原文
         │   │   ├─ 阅读理解: 每题标注原文段落+具体句子, 推理题额外给出推理逻辑链
         │   │   │   格式: "原文第 N 段: '...' → 由此推断 ... → 排除 A/C/D 因为 ..."
         │   │   ├─ 听力: 每题标注 transcript 对应句子 + 推理逻辑
         │   │   │   格式: "M说: '...' → 表明 ... → 选 B"
         │   │   ├─ 完形/语法填空: 标注上下文依据句 + 语法规则
         │   │   ├─ 续写: 评分维度对应范文具体段落 ("情绪转折见 P1 第 3 句")
         │   │   └─ 应用文: 得分点对应范文具体位置 ("要点 1 见第 2 段首句")
         │   ├─ D2. 答案 B 偏修复 (ABCD 均匀化, 每类题型 ≈25%±5%)
         │   ├─ D3. 275 rule_synth 题: 有价值的补 analysis, 低质的淘汰替换
         │   ├─ D4. 难度梯度重建: 阅读 A(easy)→D(hard), 完形前易后难 (P9)
         │   ├─ D5. 31/40 讲义补 hook + relations 段, 达到 7/7 结构完整
         │   ├─ D6. 全局讲解与原文对应 — 教案中引用的知识点必须标注教材出处+真题考过的具体题号
         │   └─ D7. 该重写的重写, 可优化的优化 — 以模型输出为唯一标准, 不保留"凑数"内容
         └─ Phase E: 全系统审计 → 目标每项 100/100
             ├─ 数据 78→100 (逐题入库 + 图谱连通)
             ├─ 内容 42→100 (解析对应原文 + ABCD均匀 + 梯度 + 7段完整)
             └─ 工具 72→100 (全模块宪法合规 + 缺失工具补齐)
