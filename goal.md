...
- 关键结论：
  ...
  - 未改动新功能。
- 结论：审计系统口径已闭环；待 `docs/user_test_round1.md` 与 `docs/teacher_feedback_round1.md` 补录真人反馈后正式进入 M4。
...
- 一次里程碑 = 一次完整闭环，禁止将同一问题切成多轮小修。
- 每个里程碑只在三件事完成后结束：执行 -> 复算 -> 同步（`goal.md` 与 `docs/data_accuracy_audit.md`）。
- `FAIL`/新增 `WARN` 一律阻断主链路，必须先修再进下一里程碑。
- 所有可复算产物必须入 `data/reports/`，并带 `run_id`。
...
#### 一、M3 闭环收口（本次会话：先交付，不再新增功能）

- 目标：把审核链路完全统一，并补齐 M3 最后未闭环材料，形成可复验的单一交付面。
- 交付边界（一次性）：
  - 产出 `docs/user_test_round1.md` 与 `docs/teacher_feedback_round1.md`（真人反馈有/无均给替代方案与排期）。
  - `goal.md`、`docs/data_accuracy_audit.md`、`data/reports/m3_closure_*.json|md` 三套文档完成状态对齐。
  - 形成 `M3` 里程碑冻结包（run_id、验收结果、剩余风险、下步动作），并写入 `goal.md` 的“里程碑执行纪律”入口。
- 一次验收条件：
  - `scripts/data_accuracy_check.py` 与 `scripts/stop_gate.sh` 同时 PASS。
  - `data/reports/verification_protocol.json` 的 V1~V8 均有明确反馈项，不允许 `pending`。
  - `M3` 证据文件、run_id、结论在三处文档完全一致。

#### 二、M4 课程主链路交付闭环（M3 完成后一次启动）

- 目标：一次性交付 40 节“可教闭环”，覆盖教学、题库、关联、作业、前端可用性。
- 交付边界（一次性）：
  - `course_templates.yaml` 与课程主链路完整冻结，`courses=40`、`course_materials` 口径稳定。
  - `R1~R6` 一次性重跑并写入里程碑报告（含不合格条目修复闭环）。
  - `/app` 上课中台（讲义/作业/关联）与学生答题（弱点/复测）链路打通并形成 1 轮回放演示脚本。
- 一次验收条件：
  - `R1~R6` 无可阻断 FAIL，允许的说明型边界必须有收口日期。
  - 课程相关 API 与前端入口可重放；课程材料映射可按课程-知识点-题目完整 trace。
  - `question_bank` 与 `course_materials` 在一次运行中不再出现新断裂风险。

#### 三、M5 运营试运行闭环（课程交付后一次性演练）

- 目标：形成可复现运营启动包，不做“后续再补”式交付。
- 交付边界（一次性）：
  - 启动与巡检文档、备份/恢复脚本、告警流程固定化（单点可复现）。
  - 老师/学生关键流程（课程选择、答题、弱点 drill）各录入 1 次真实演练证据。
  - `data_accuracy_check` 与 `stop_gate` 固化为每周例行执行清单。
- 一次验收条件：
  - 至少一次“连续流程”演练不出现阻断。
  - `data_accuracy_check` 与 `stop_gate` 周更后维持稳定通过。
  - 运营文档与演练材料同步上链到 `data/reports/` 与 `docs/data_accuracy_audit.md`。

### 里程碑执行纪律（本文件新增）

1. 按顺序推进：`M3 -> M4 -> M5`，不允许交叉切换。
2. 每个里程碑只执行“一次主脚本链”：`数据修订 -> 全量验收 -> 文档冻结 -> 状态更新`。
3. 任何新发现 FAIL/WARN，只能在当前里程碑内修完并重跑，不允许进入下一级里程碑。
4. 里程碑结束前不更新为“已完成”，必须具备：
   - run_id
   - 结果快照（json/md）
   - 下一步可直接执行清单
...
- 2026-06-10 ~ 2026-06-16：M3 闭环收口（重点）
- 2026-06-17 ~ 2026-07-07：M4 课程闭环
- 2026-07-08 ~ 2026-07-14：M5 试运行闭环与交付冻结
