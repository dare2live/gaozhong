...
- **🚀 [GOAL] Phase E 初中板块全深度复刻启动 (用户2026-07-07拍板"全深度复刻,按高中顺序推进")**: 以中考为结果, 用小学/初中教材·课标·考纲(比照高考侧方法论)辅导初中生, 最后用deepens式连续视图打通初中→高中。现状差距(高中↔初中): exam_questions 190(2015-2026) vs zhongkao_questions 90(仅2024-2025); tests_word 27805/tests_grammar 88/exam_point 608 边 vs 初中0; sections 470 vs 初中(修复前)0; courses 40 vs 初中0。拆5个子任务(E1-E5), 按高中当初顺序(L1地基→L2关联→L3课程→打通串联):
  ...
  - **🚧 E4 初中40节课程框架 进行中 — 用户"仔细研究"后重新定向(2026-07-08)**: 用户纠正两轮方案(①"高中40节其实是命题频次驱动的复习课非教材单元课" ②"骨架不是教材单元也不是硬凑40节, 应该是初高中知识体系+数据血缘, 从知识点追溯到具体年级/册次/年份真题, 40节是从这个体系派生的临时数字, 不要硬编码, 遵循模块+数据+配置文件")。已完成的地基工作:
    ...
    - 待续: vocab/phrase维度的unit-level lineage(hujiao_vocab.jsonl现只有grade粒度无unit_number, 需仿造grammar_occurrence同法从Vocabulary section原文提取) + 知识点详情查询函数(整合grammar/vocab/phrase三轴lineage+K12衔接+中考印证) + 真正的config驱动"N节"生成器(N从新config key读, 不硬编码, 组织轴用真实知识点重要性分层非命题频次)。
