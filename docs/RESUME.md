...
1. **教材基石完整提取(真地基, 最高优先)**: 外研选必4 仍 0 单元、教材覆盖课标 ~46%。**这是重建任何上层教学内容的前置**——数据基石优先(§1.1)。
2. **趋势/考点模型重建(核心竞争力)**: 真题已清洗(EOL 入库 + provenance 正确 + 去停用词), 旧 trend_engine/exam_patterns 训练在污染数据上的结论已不可信, 可在干净数据上重建 考点趋势/热力图/关联(用户 MEMORY 核心竞争力)。
3. **M0 收口剩余**: 2021 听力答案源是 Sohu 候选(非官方 EOL 答案表), analysis 已留 lineage; 写作 rescope 未入(需 writing schema)。
4. **CC 减债**: 仍 37 个 CC>10 函数(基线对齐现状), 可继续降。
