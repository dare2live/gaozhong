...
## 📣 对外口径卡 (2026-06-22, 交付必带 — 防脱离前端的PPT/口头丢caveat)

> 命题趋势=核心竞争力卖点, **但卖点真值内核是结构层(structural_truth/explicit_label), 不是题材层(LLM)**。
> 项目的 provenance 纪律(每层标真相源等级 + 前端逐条 carry caveat + 三门锁)是它**敢对"D0=100%准"学校付费场景对外**的底气。对外任何材料按此三档措辞:

| 档 | 内容 | 可对外措辞 |
|---|---|---|
| ✅ **可断言真值** | ① 题型presence结构迁移(短改退场/读后续写·应用文新增, `structural_truth`, 教育部新高考改革+真题原卷双源) ② 词汇热力四象限(超纲但考过142词必教★, 课标∩真题双可验集合) ③ cognitive_skill技能侧(`explicit_label` 教研显式题型: 应用文100%找信息/说明文记叙文最考推断) | "真题原卷/教研解析的结构性事实" 可直接断言 |
| ⚠️ **方向性参考(必标LLM)** | genre/theme 题材分布(481边全 `dual_model_agree` 零人工核验, 坑16双模型0分歧≠对) + 设问×题材的**题材侧** | 必标 "LLM辅助分类·方向性参考·非考纲官方"; 当导航不当结论 |
| 🚫 **不可下结论** | ① 逐年趋势slope(辽宁仅5年/2023=6题, `trend_reliable=false`) ② cognitive_skill新era**精确占比**(2021+仅2023单年n=15, `distribution_reliable=false`) | "命题哲学迁移(推断28→47%)" 只说**方向**, 必配 "新高考侧样本仅2023单年n=15, 方向性非精确"; 逐年斜率不画 |

**一句话**: 卖"结构层真值 + 词汇热力 + 设问技能"敢拍胸脯; genre/theme 题材层和单年n=15的迁移精度**只能当方向性参考**, 标 LLM/样本量, 绝不当真值卖 — 这正是不砸"D0=100%"招牌的底线。
