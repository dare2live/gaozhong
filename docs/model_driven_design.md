...
| V2 | 凭感觉决定考点权重 | 用模型计算的 weighted slope + weighted frequency |
...
## 正向铁律 (与违宪清单互补 — 这些是必须做到的)

| # | 铁律 | 实施 |
|---|---|---|
| **P1** | **年份权重递增** | 2025(权重5) > 2024(4) > 2023(3) > 2022(2) > 2021(1.5) > 旧(0.5). 趋势计算/频次统计/模板选取全部加权 |
| **P2** | **命题趋势优先** | 上升考点 (如词义猜测 +0.179) 必须加大出题比重; 下降考点 (如细节理解 -0.179) 主动减少 |
| **P3** | **近年模板为准** | 提问句式/干扰项设计/难度分布/文章选题以 2023-2025 真题为唯一模板, 不回溯旧年份 |
